...
if (typeof SimileAjax == "undefined") {
    ...
    // dacă nu există obiectul SimileAjax creează-l
    const SimileAjax = {
        loaded:                 false,
    ...
    let getHead = function getHead (doc) {
        return doc.getElementsByTagName("head")[0];
    };
    
    SimileAjax.findScript = function findScript (doc, substring) {
        let heads = doc.documentElement.getElementsByTagName("head");
        for (let h = 0; h < heads.length; h++) {
            let node = heads[h].firstChild;
            while (node != null) {
    ...
    SimileAjax.includeJavascriptFile = function includeJavascriptFile (doc, url, onerror, charset) {
        onerror = onerror || "";
        if (doc.body == null) {
            try {
